...
export default function VotePage() {
  ...
  useEffect(() => {
    ...
    predictionService.getByShareCode(shareCode)
      .then(p => {
        ...
        // Vérifier si le joueur a le boost Double Vote (seulement si les boosts sont activés)
  ...
  // Réinitialiser le second vote si le premier change ou si on désactive le double vote
  ...
  const handleVote = async () => {
    ...
    try {
      await predictionService.vote(
        prediction.id,
        selected.id,
        useDoubleVote ? secondSelected?.id : undefined,
      )
      const label = useDoubleVote && secondSelected
        ? `${selected.label} + ${secondSelected.label}`
        : selected.label
      setVotedLabel(label)
      setVoted(true)
  ...
  if (voted) return (
    <div style={pageStyle} className="flex items-center justify-center px-4">
      <div className="text-center max-w-sm">
        ...
        <p className="font-bold text-lg" style={{ color: '#c8880c', fontFamily: '"Cinzel", serif' }}>
          « {votedLabel} »
        </p>
  ...
  // Options disponibles pour le second choix (exclut le premier)
      ...
      <div className="max-w-xl mx-auto px-4 py-10">
        ...
        <div className="relative p-6 rounded shadow-2xl" style={{ background: '#161209', border: '1px solid #6b5010' }}>
          ...
          <h1 className="text-2xl font-extrabold leading-snug mb-2"
            style={{ fontFamily: '"Lora", serif', color: '#f0dfa8' }}>
            « {prediction.question} »
          </h1>
          {prediction.isAnonymous && (
          ...
          {/* ── Double Vote toggle ── */}
          {hasDoubleVote && (
            <div className="mb-4 flex items-center gap-3 px-3 py-2.5 rounded"
              style={{ background: '#1a1208', border: '1px solid #6b5010' }}>
              <button
          ...
          <p className="text-sm font-semibold mb-3" style={{ fontFamily: '"Cinzel", serif', color: '#8a7a5a' }}>
            {useDoubleVote ? 'Ton premier choix' : 'Soumets-toi au jugement d\'Orakl'}
          </p>
          <OptionList
          ...
          <div className="mt-6">
            ...
            <button
              onClick={handleVote}
              disabled={!selected || isSubmitting || (useDoubleVote && !secondSelected)}
              className="w-full font-bold py-3.5 rounded text-sm transition flex items-center justify-center gap-2"
              style={{
                background:  (selected && (!useDoubleVote || secondSelected) && !isSubmitting)
                  ? 'linear-gradient(135deg, #a36808, #c8880c, #e6a817)' : '#2a2218',
                color:       (selected && (!useDoubleVote || secondSelected) && !isSubmitting)
                  ? '#0e0c08' : '#3a2d10',
                fontFamily:  '"Cinzel", serif',
                fontSize:    '0.8rem',
                border:      `1px solid ${selected ? '#f5c842' : '#2a2218'}`,
                cursor:      (selected && (!useDoubleVote || secondSelected) && !isSubmitting)
                  ? 'pointer' : 'not-allowed',
            ...
            >
              ...
              {isSubmitting
                ...
                : useDoubleVote && selected && secondSelected
                  ? `✦ Double vote — « ${selected.label} » & « ${secondSelected.label} »`
                  : selected
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="font-semibold text-sm"
                  style={{ color: isSel ? '#f5c842' : '#f0dfa8', fontFamily: '"Cinzel", serif' }}>
                  {option.label}
                </p>
                {option.description && (
